...
ssid="$(nm_wifi_ssid)"; kernel="$(uname -r 2>/dev/null)"
{
  ...
  echo "site       : $NM_SITE"
  echo "source_if  : ${NM_SRC_IFACE:-}"
  echo "source_ip  : ${NM_SRC_IP:-}"
...
ctx_header="$(nm_ctx_header),source_interface,source_ip,interface,local_ip,gateway,wifi_ssid,os,kernel"
ctx_row="$(nm_ctx_row),$(nm_csv_join "${NM_SRC_IFACE:-}" "${NM_SRC_IP:-}" "$iface" "$lip" "$gw" "$ssid" "$NM_OS" "$kernel")"
